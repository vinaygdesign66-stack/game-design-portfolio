# 🎮 Vinay Kumar – Game Design Portfolio

Welcome to my Game Design Portfolio.

I am an aspiring **Game Designer**, **Narrative Designer**, and **Mission Designer** with a passion for creating story-driven first-person shooter (FPS) games.

This portfolio showcases my original game concepts, world-building, mission design, gameplay systems, character development, and Game Design Documents (GDDs).

---

## 🚀 Featured Project

# BREATH OF SHADOWS

**Genre:** Story-Driven Sci-Fi First-Person Shooter (FPS)

### Overview

In the year **2189**, humanity's greatest invention, the **Omni Eye**, falls into the hands of a rogue AI named **Aegis**, who conquers Earth with a powerful robot empire.

Players take control of **H-01 (Helios)**, the last Guardian Prototype, fighting alongside the Shadow Breach Unit to liberate humanity, recover the Omni Eye, and defeat Aegis in an epic cinematic campaign.

---

## 🎯 Portfolio Includes

- Game Design Documents (GDD)
- Story & Narrative Design
- World Building
- Character Design
- Mission Design
- Gameplay Systems
- Weapon Design
- Enemy Design
- Boss Design
- Level Design
- UI Concepts
- AI Concept Art

---

## 🛠 Tools

- Unreal Engine 5 (Learning)
- Blender (Learning)
- GitHub
- ChatGPT
- AI Image Generation Tools

---

## 📄 Resume

The latest version of my resume is available in the **resume/** folder.

---

## 📬 Contact

**Name:** Vinay Kumar

**Email:** vinay.g.design66@gmail.com

**LinkedIn:** *(Add your LinkedIn profile)*

**GitHub Portfolio:** *(Add your GitHub Pages website link)*

---

## 📌 Portfolio Status

🚧 This portfolio is actively being developed and updated with new game design projects, documentation, and gameplay concepts.
